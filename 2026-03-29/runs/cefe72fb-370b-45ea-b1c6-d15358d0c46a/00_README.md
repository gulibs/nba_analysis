...
- **赛前单场**：**04a→04b→04c→04d0（ESPN 外部验证）→04d（结论）**；**赛后单场（已结束）**：**04a（盲事实）→04b→04c→04d0→04d（先验分析）→04e（真实赛果）→04f（对照复盘）**。**禁止**在 04a–04c 写出最终信心分与投注式推荐（见 `01_rules.md`）；**04d0** 仅作第三方公开信息交叉核对，**不**替代数据包盘口事实。
...
4. 对每个 `game_id` **依 manifest 顺序**执行单场步骤（赛前：04a–04c、**04d0**、04d；赛后：04a–04c、**04d0**、04d–04e–04f），再进入下一个 `game_id`；勿合并多场于一轮。
5. 将各场 **04d（赛前）或 04f（赛后）摘要** 带入 `05`，合成串关。
6. 执行 `06` 统一排版（可选）。

## 本包元信息
- 赛程日: `2026-03-29`
- scope game_ids: `[7361, 7362, 7363, 7364, 7365, 7366]`
- 生成 UTC: `2026-03-28T15:40:29.794506+00:00`
- 赔率最近入库: `2026-03-28T23:19:31.067252+08:00`
...
**`pick_zh` 与各玩法方向**（勿仅用 `game_id` 指代场次）：大小分须写 **大分/小分** + 总分线；胜负须写 **主胜/客胜** 或 **某队胜**；胜分差须写 **主/客胜 + 档位**；让分须写 **让分主胜/让分主负** + **客队「`away_team`」vs 主队「`home_team`」（±线）**（与数据包 JSON 一致，勿对调主客；详见 `01_rules.md` 第八节表格）。
